...
class CBOFuturesDates:
    def __init__(self):
        ...
        self.valid_days_set=frozenset(self.valid_days.dt.date)
...
def read_csv_future_files(vixutil_path:Path)->pd.DataFrame:
        ...
        #remove them.
        #on the settlement day of contracts expiring before 2015, CBOE recorded open, high, low and
        #close as 0 and filled in only the settlement price, which is the final value of the contract.
        #keep those rows, with the settlement price as the close and the other prices missing, so the
        #last day of a contract is not lost.  rows of 0s on other days are days the contract did not
        #trade, and are still removed.
        #contracts that never traded also get a settlement price on their expiry day, with no volume
        #and no open interest.  they are not kept, so no contract consists of a settlement row alone.
        settlement_day=(futures_frame_ordered_cols["Close"]==0) \
            & (futures_frame_ordered_cols["Settle"]>0) \
            & (futures_frame_ordered_cols["Trade Date"]==futures_frame_ordered_cols["Expiry"]) \
            & ((futures_frame_ordered_cols["Open Interest"]>0) | (futures_frame_ordered_cols["Total Volume"]>0))
        futures_frame_ordered_cols.loc[settlement_day,['Open','High','Low']]=np.nan
        futures_frame_ordered_cols.loc[settlement_day,'Close']=futures_frame_ordered_cols.loc[settlement_day,'Settle']
...
def write_futures_contracts_csv(vix_futures_records:pd.DataFrame,out_dir:Path)->list[Path]:
    """
    Write the futures history to one csv file per contract (expiry), named
    VX_<expiry>_monthly.csv or VX_<expiry>_weekly.csv, sorted by trade date.
    Returns the list of files written.
    parameters:
    -----------
    vix_futures_records:  the futures history in records format, as returned by load_vix_term_structure.
    out_dir: folder to write the files to.  It is created if it does not exist.
    """
    out_dir=Path(out_dir)
    make_dir(out_dir)
    written=[]
    for expiry,contract in vix_futures_records.groupby("Expiry"):
        kind="weekly" if contract["Weekly"].iloc[0] else "monthly"
        fn=out_dir/f"VX_{expiry.date().isoformat()}_{kind}.csv"
        contract=contract.sort_values("Trade Date").round({c:4 for c in _value_columns})
        contract.to_csv(fn,index=False,date_format="%Y-%m-%d")
        written.append(fn)
    return written
