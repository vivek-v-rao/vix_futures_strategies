...
parser.add_argument("--contracts",
                    metavar="output_dir",
                    dest="contracts_dir", help="""write the history of each vix futures contract (weekly and monthly expiries)
                    to its own csv file in output_dir.""")
...
def main():
    ...
    if out_dir := args.contracts_dir:
        files=vix_utils.write_futures_contracts_csv(vix_futures, out_dir)
        print(f"Wrote {len(files)} contract files to {out_dir}")

    other_outputs=(args.futures_records, args.futures_wide, args.spot_records, args.spot_wide, args.w_m1m2, args.futures_m1m2)
    if not any(other_outputs):
        return 0
