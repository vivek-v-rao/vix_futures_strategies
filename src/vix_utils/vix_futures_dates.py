...
def vix_futures_expiry_date_monthly(year: int, month: int):
    ...
    friday_expiration : bool = any(_valid_cfe_days.isin([pd.Timestamp(option_expiry_date)]))
    ...
    if friday_expiration and not any(_valid_cfe_days.isin([pd.Timestamp(futures_expiry_date)])):
...
def vix_constant_maturity_weights(vix_calendar : pd.DataFrame, start_date : str|pd.Timestamp=None, end_date : str|pd.Timestamp=None) -> pd.DataFrame:
    ...
    #these hold dates, so make them datetime columns rather than filling float NaN with timestamps
    df_foo[start_roll_col] = pd.Series(pd.NaT, index=df_foo.index, dtype="datetime64[ns]")
    df_foo[end_roll_col] = pd.Series(pd.NaT, index=df_foo.index, dtype="datetime64[ns]")
